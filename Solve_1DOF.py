...
for i, icost in enumerate(cost_functions[:-1]):
    ...
    for ijoint in range(n_joint):
        # problems.append(set_problem_ocp(qLow[ijoint], qUpp[ijoint],
        #                                 dqMax[ijoint], tNode,
        #                                 qNode[ijoint], nGrid))
        # problems[ijoint].solve(100000)
        problems.append(set_problem(qLow[ijoint] ,qUpp[ijoint],
                                    dqMax[ijoint], tNode,
                                    qNode[ijoint], nGrid))
        n = 15
        d = [0, 0.8]
        isol = problems[ijoint].solve(icost, n, d, 5000)
        ...
        dsol = pd.DataFrame({'Time': isol[0],
                             'q': (isol[1]-isol[1][0])/np.diff(qNode).flatten()[0], #np.rad2deg(isol[1]),
                             'dq': L1*isol[2], })
        dsol = dsol.assign(Cost=icost).assign(Joint=ijoint)
...
# params, extras = curve_fit(lognpdf, data_cost_smooth[0].Time, y1, bounds=((-3.0, 0.1, 0), (-1.5, 0.6, 1e-2)))
...
print(f'mu {param_log[0]}, sigma {param_log[1]}, t0 {param_log[3]} and  D {param_log[2]}')
