...
def main(n_shooting=120, final_time:float=0.3, alpha_beta:float=5000,  t0:float=0):
    ...
    obj_dict = {'Minimize jerk':{'cost_dt2':False, 'cost_marker':True,
                ...
                'Minimize jerk and time':{'cost_dt2':True, 'cost_marker':True,
                                          'min_energy': False, 'min_jerk':True},
                'Minimize jerk, energy and time':{'cost_dt2':True, 'cost_marker':True,
    ...
    for i, irow in marker_position.iterrows():
        ...
        pos = np.hstack([
            ...
            ).to_array()[:-1] for j in range(model.nbMarkers())])

        wrist_vel = model.markersVelocity(
            np.deg2rad(all_sol[i_obj][['q1', 'q2']].values.T[:, i_frame]),
            all_sol[i_obj][['dq1', 'dq2']].values.T[:, i_frame]
        )[marker_idx].to_array()[:-1]
        ...
        marker_dot = np.hstack([
            np.linalg.norm(
                model.markersVelocity(
                    np.deg2rad(all_sol[i_obj][['q1', 'q2']].values.T)[:, i_frame],
                    all_sol[i_obj][['dq1', 'dq2']].values.T[:, i_frame])[j].to_array()
            ) for j in range(model.nbMarkers())])

        marker_ddot = np.hstack([
            np.linalg.norm(
                model.markerAcceleration(
                    np.deg2rad(all_sol[i_obj][['q1', 'q2']].values.T[:, i_frame]),
                    all_sol[i_obj][['dq1', 'dq2']].values.T[:, i_frame],
                    all_sol[i_obj][['ddq1', 'ddq2']].values.T[:, i_frame])[j].to_array()
            ) for j in range(model.nbMarkers())])


        marker_position.loc[i] = np.hstack(([irow.Time], #'Time'
                                            pos, #['Elbow_x', 'Elbow_y', 'Wrist_x', 'Wrist_y']
                                            wrist_vel, # ['Wrist_x_vel', 'Wrist_y_vel']
                                            marker_dot, # ['Elbow_vel', 'Wrist_vel']
