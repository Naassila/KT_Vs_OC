...
qNode = np.deg2rad([[30, -70], # Q1 start and stop
                    [60, 10]]) # Q2 start and stop
...
for i, icost in enumerate(cost_functions[:]):
    ...
    dsol = pd.DataFrame({'Time':isol[4],
                             'q':isol[5][0],#np.rad2deg(isol[4][0]),
                             'dq':isol[6][0],
                             'ddq':isol[7][0]})
    ...
    ijoint = 1
    problems.append(set_problem(qLow[ijoint]+L1/L2*qLow[0], qUpp[ijoint]+L1/L2*qUpp[0],
                                dqMax[ijoint]+L1/L2*dqMax[0], tNode,
                                qNode[ijoint]+L1/L2*qNode[0], nGrid, ))
    ...
    dsol = pd.DataFrame({'Time': isol[4],
                         'q': isol[5][0]-L1/L2*data_cost_smooth[2*i].q,  # np.rad2deg(isol[4][0]),
                         'dq': isol[6][0]-L1/L2*data_cost_smooth[2*i].dq,
                         'ddq': isol[7][0]-L1/L2*data_cost_smooth[2*i].ddq})
    dsol = dsol.assign(Cost=icost).assign(Joint=ijoint)
    data_cost_smooth.append(dsol)

    velocity = pd.DataFrame({'Time': isol[4]})
    ...
    dsol = pd.DataFrame({'Time': isol[0],
                         'q': isol[1] - L1 / L2 * data_cost_discrete[2 * i].q,  # np.rad2deg(isol[1]),
                         'dq': isol[2] - L1 / L2 * data_cost_discrete[2 * i].dq,
                         'dddq': isol[3] - L1 / L2 * data_cost_discrete[2 * i].dddq})
    dsol = dsol.assign(Cost=icost).assign(Joint=ijoint)
    data_cost_discrete.append(dsol)
...
# g.axes[1][0].set_ylim(-320, 0)
# g.axes[1][0].set_yticks([-320, -160, 0])
g.axes[1][0].set_ylabel(var_list[1])

# g.axes[1][1].set_ylim(-160, 0)
# g.axes[1][1].set_yticks([-160, -80, 0])
